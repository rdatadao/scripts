...
const maxBlock = 13977565;

const sleepTime = 500; //1000 = one second
//end **********parameters**************************
...
async function listenToEvents(fromBlock: number, toBlock: number, querySize: number) {
    ...
    while (fromBlock <= toBlock) {
        ...
        console.log(`Number of events from ${fromBlock} to ${fromBlock + querySize}: ${events.length}, totalEventsNumber: ${totalEventsNumber}`);
...
async function main() {
    await listenToEvents(minBlock, maxBlock, 1000
    ).catch(error => console.error('Error listening to events:', error));
}
